...
@section('content')
    ...
    <form class="block block-form" method="post" action="{{route('helpDisk.update',$helpDisk->id)}}">
        @csrf
        @method("PUT")
        <div class="block-content block-content-full">
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        <label class="cp">Vorname <sup>*</sup></label>
                        <select name="page_id" id="" class="form-control js-select2" required>
                            <option value="{{$helpDisk->page->id}}" selected>{{$helpDisk->page->displayed_name}}</option>
            ...
            <div class="row">
                <div class="col-md-4">
                    <textarea id="summernote" name="details" required>{!! $helpDisk->details !!}</textarea>
                    @error('details')
                    <small class="form-text text-danger">{{$message}}</small>
                    @enderror
                </div>
